...
from speech_utils import listen_print_loop, transcribe, testtrans, MicrophoneStream
...
BoxLayout:
    ...
    BoxLayout:
        ...
        Button:
            ...
            background_normal: ''
            background_color: (190/255,10/255,117/255,.2)
...
class RemembranceAgent(App):
    ...
    def pasteContentDelayed(self, dt):
        if self.queryWords:
            ...
            labelResult = Factory.MyLabel(text='%s%s' % ("First Match: ", self.top_results[TOP_K - 1] + "\n"))
            self.root.ids.target.add_widget(labelResult)
            labelResult = Factory.MyLabel(text='%s%s' % ("Second Match: ", self.top_results[TOP_K - 2] + "\n"))
            self.root.ids.target.add_widget(labelResult)
            labelResult = Factory.MyLabel(text='%s%s' % ("Third Match: ", self.top_results[TOP_K - 3] + "\n"))
            self.root.ids.target.add_widget(labelResult)
        else:
            Clock.schedule_once(self.pasteContentDelayed, 1.5)
